...
So create a shallow copy of Julia's array, and remove the cat ages from 
...
function checkDogs(dogsJulia, dogsKate){
    let corectArr = dogsJulia.slice(1, -2)
    let allAges = corectArr.concat(dogsKate)
    let dogNum = 1
    for(let i of allAges){
        if(i > 3){
            console.log(`dog number ${dogNum++} is an adult, and is ${i} years old`)
        }else{
            console.log(`dog number ${dogNum++} is still a puppy, and is ${i} years old`)
        }
    }
    return allAges;
}
